...
//Current 'Specials'
const specials = [
    { pancake: 'Cherry Bomb', cost: 12, special: true },
    { pancake: 'Pumpkin Spice Cinnamon Roll', cost: 11, special: true },
    { pancake: 'Banana Swirl', cost: 10, special: true },
];
...
pancakeType.addEventListener('change', () => {
    ...
    const pancakeId = Array.from(pancakeTypeOptions).find(
        (a) => +a.value === basePrice
    );
...
const deliveryChoices = document.querySelectorAll('[name="delivery"]');
deliveryChoices.forEach((choice) => {
    choice.addEventListener('click', () => {
        choice.id === 'delivery' ? (deliveryCost = 5) : (deliveryCost = 0);
        summaryTotalCost();
    });
});
...
const [specialsBtn, customBtn, viewCartBtn] = document.querySelectorAll(
    '#specials, #custom, #viewCart'
);
const [specialsPage, customPage, summary] = document.querySelectorAll(
    '.specials, .customize, .summary'
);
const [homeBtn, redirectAboutBtn] = document.querySelectorAll(
    '#homeBtn, #redirectAboutBtn'
);
const [home, order, confirmation, about] = document.querySelectorAll(
    '.home, .order, .confirmation, .about'
);
...
specialsBtn.addEventListener('click', () =>
    toggleOrderPage(specialsPage, specialsBtn)
);
customBtn.addEventListener('click', () =>
    toggleOrderPage(customPage, customBtn)
);
...
redirectOrderBtns.forEach((button) =>
    button.addEventListener('click', () => togglePage(order))
);
...
function isInCart(pancake, special = false) {
    ...
    return cart.findIndex(
        (item) => JSON.stringify(item.pancake) === JSON.stringify(pancake)
    );
}
...
function displayOrder() {
    ...
    if (cart.length < 1) {
        ...
        cart.forEach((item) => {
            let typeOfPancake;
            if (!item.isSpecial()) {
                let [base, toppings, extras] = [
                    item.pancake.pancakeBase,
                    item.pancake.toppings,
                    item.pancake.extras,
                ];
...
const [orderAddBtn, orderRmBtn] = document.querySelectorAll(
    '.orderAdd, .orderRm'
);
...
function resetPancake() {
    ...
    pancakeType.value = 5;
    document
        .querySelectorAll('input[name="topping"], input[name="extra"]')
        .forEach((topping) => {
